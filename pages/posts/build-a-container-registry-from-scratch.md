...
This article tries to answer the following question—"What happens when you do `podman push/pull`?" We'll do so by creating a simple container registry--we'll call it [tiny-registry](https://github.com/danishprakash/tiny-registry)—using pure Go under 200 lines that will respond to push and pull commands from any container engine (we'll be using podman for our use case).
